...
interface ShaderMaterialProps extends ShaderProps {
  hovered?: boolean;
}
...
const DotMatrix: React.FC<DotMatrixProps> = ({
  ...
  const shaderSource = useMemo(() => createShaderSource(shader, center), [shader, center]);
...
const ShaderMaterial: React.FC<ShaderMaterialProps> = ({
  ...
  const material = useMemo(() => {
    try {
      ...
      return new THREE.MeshBasicMaterial({ color: "red" }); // Fallback material
    }
  }, [source, getUniforms]);
...
const createShaderSource = (shader: string, center: ("x" | "y")[]) => `
  precision mediump float;
  in vec2 fragCoord;
